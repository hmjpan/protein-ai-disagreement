...
strict-set proteins (rank-averaged ensemble): 0.968; standard-homology
threshold sensitivity in Table S4.
...
| Strict non-overlap, all proteins (uniform, rank-averaged) | 2,490 | 0.968 | - | - |
...
AUROC 0.968); 50% and 30% identity thresholds exclude 46 and 82 proteins
with medians 0.968 and 0.968. The 8-mer heuristic used in the previous
...
**Table S10. Matched two-family control (DMS versus clinical).** Assay- or
protein-level Spearman between |U_seq - U_evo| disagreement and the
two-family ensemble error, identical definition in both datasets, with
10,000-resample bootstrap CIs; scale-matched AUROC comparison (high-error
versus misclassification).
