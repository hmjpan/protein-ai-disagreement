...
def main():
    ...
    cdf.to_csv(STATISTICS / "uniform_2x2_per_protein.csv", index=False)
    t22 = []
    for name, sel in [
            ("full scored set (both labels, >= 10 variants, five-model complete)",
             ~np.zeros(len(cdf), bool)),
            ("strict gate-complete subset", cdf["in_gate_complete"])]:
        sub = cdf[sel]
