...
The REVERSAL of the disagreement-error association in clinical data is as informative as the weak DMS association. DMS assays measure one well-defined phenotype under controlled conditions [41]; clinical labels aggregate heterogeneous phenotypes, cell types and genetic backgrounds curated across submitters [23]. The boundary condition we document -- a weak disagreement-error association in fitness landscapes that inverts in clinical annotation, even when the two-family model configuration is matched -- prevents mechanical misapplication of disagreement-based confidence in clinical genomics: agreement of sequence and evolutionary models does not certify a variant call as reliable.
