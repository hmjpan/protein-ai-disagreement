...
[61] Eddy S R 2011 Accelerated profile HMM searches PLoS Comput. Biol. 7 e1002196
...
while _i < len(_lines):
    ...
    if re.match(r"^Figure [1-6]\. ", _line):
        ...
        if cf.exists():
            ...
            _w2, _h2 = _I2.open(cf).size
            doc.add_picture(os.fspath(cf),
                            width=Inches(min(6.0, 8.5 * _w2 / _h2)))
       ...
       or _line.startswith("Figure 5.") or _line.startswith("Figure 6."):
        ...
        if cf.exists():
            try:
                ...
                _w3, _h3 = _I3.open(cf).size
                doc.add_picture(os.fspath(cf),
                                width=Inches(min(6.0, 8.5 * _w3 / _h3)))
            except Exception:
                pass
        add_rich_paragraph(doc, _line)
        _i += 1
...
try:
    ...
    _t3 = "\n".join([
        ...
        "|---|---|---|---|---|",
        f"| Full clinical benchmark, gate-complete (uniform = five-model) | {len(_cf)} | "
        f"{_med(_cf, 'auroc_uniform'):.3f} | {_med(_cf, 'auroc_biogate'):.3f} | "
        f"{_med(_cf, 'auroc_best'):.3f} |",
        f"| Strict non-overlap, gate-complete (uniform = two-family centroids) | {len(_cs)} | "
    ...
    for _gn in sorted(int(_m.group(1)) for _fn in os.listdir(cdir)
        ...
        _fw, _fh = _Img.open(cdir / f"Fig{_gn}_full.png").size
        _wid = min(6.2, 8.5 * _fw / _fh)
        _blocks.append(_pbr)
