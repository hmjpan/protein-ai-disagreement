...
def main():
    ...
    sizes = rl.groupby("UniProt_ID")["plddt"].transform("size")
    rp = rl[sizes >= 50]
    mpf = sm.OLS(rp["D_evo_struct_dm"],
                 sm.add_constant(rp[["plddt_dm", "disorder_dm", "contact8_dm",
                                     "norm_pos_dm", "is_helix_dm",
                                     "is_sheet_dm", "msa_low_dm",
                                     "msa_high_dm"]])
                 ).fit(cov_type="cluster",
                       cov_kwds={"groups": rp["UniProt_ID"]})
    cip = mpf.conf_int().loc["plddt_dm"]
    rows = [{"model": "pLDDT only",
             ...
             "r2": m0.rsquared}]
    rows.append({"model": "full covariates (primary >=50-residue proteins)",
                 "beta_plddt": mpf.params["plddt_dm"],
                 "ci_low": cip[0], "ci_high": cip[1],
                 "se_plddt": mpf.bse["plddt_dm"],
                 "p_plddt": mpf.pvalues["plddt_dm"],
                 "n": int(mpf.nobs),
                 "n_proteins": int(rp["UniProt_ID"].nunique()),
                 "r2": mpf.rsquared,
                 "beta_disorder": np.nan, "p_disorder": np.nan,
                 "beta_contact": np.nan, "beta_helix": np.nan,
                 "beta_sheet": np.nan})
    rows.append({"model": "full covariates (all residue-level rows)",
