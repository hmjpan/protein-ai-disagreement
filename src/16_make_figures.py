...
def main():
    ...
    import numpy as _np
    u2 = pd.read_csv(STATISTICS / "uniform_2x2_per_protein.csv")
    strict5 = u2.loc[u2["in_gate_complete"].astype(str) == "True",
                     "auroc_rank_mean"].to_numpy()
    if len(strict5):
        data = [tr["auroc_uniform"], tr["auroc_biogate"], tr["auroc_best"],
                strict5, non["auroc_uniform"], non["auroc_biogate"],
                non["auroc_best"]]
        labs = [f"Uniform 5-model\nfull (n={len(tr)})", "BioGate\nfull",
                "Best single\nfull", f"Uniform 5-model\nstrict (n={len(strict5)})",
                "Uniform 2-family\nstrict", "BioGate\nstrict",
                "Best single\nstrict"]
    else:
        data = [tr["auroc_uniform"], tr["auroc_biogate"], tr["auroc_best"],
